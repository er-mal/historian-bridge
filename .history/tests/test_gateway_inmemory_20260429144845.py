...
from .conftest import DEMO_TAGS
...
async def test_current(app):
    async with await _client(app) as c:
        r = await c.get("/current", params=[("tag", DEMO_TAGS[0]), ("tag", DEMO_TAGS[1])])
        assert r.status_code == 200
        body = r.json()
        assert {p["tag"] for p in body} == {DEMO_TAGS[0], DEMO_TAGS[1]}
        for p in body:
            assert "value" in p and "ts" in p
...
async def test_query_avg_interval(app):
    async with await _client(app) as c:
        ...
        assert r.status_code == 200
        body = r.json()
        # 60 points, 10-minute buckets → 6 buckets
        assert len(body) == 6
...
async def test_write_roundtrip(app):
    async with await _client(app) as c:
        r = await c.post("/write", json=[
            {"tag": "site.line2.station_c.temp", "ts": "2026-04-29T12:30:00+00:00", "value": 21.5}
        ])
        assert r.status_code == 200
